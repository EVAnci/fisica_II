...
\[
  \lambda = \frac{Q}{l} \quad \Rightarrow \quad dq = \lambda dl
\]
\begin{itemize}
...
De esta manera, un alambre fino (ideal), puede ser considerado como un cuerpo de distribución lineal, cuya densidad lineal de carga \(\lambda\) es un determinado valor. Si, a modo de ejemplo tiene un cable con una densidad lineal de carga \(\lambda = \qty{6}{\micro\coulomb\per\meter}\), significa que por cada metro de cable, hay \qty{6}{\coulomb} de carga distribuidos infinitesimalmente.
...
\]
\begin{itemize}
  \item \( \sigma \): densidad superficial, medida en Coulomb por metro cuadrado (\unit{\coulomb\per\squared\meter}),
...
\begin{example}
  Calcular la fuerza que ejerce una varilla de longitud \(\ell\), cargada con una densidad lineal de carga \(\lambda\), sobre una partícula cargada \(Q\) situada en la misma línea de la varilla y una distancia \(a\) de su extremo. La figura \ref{fig_cargas_barra_consigna} representa un esquema del enunciado.
  \begin{figure}[!ht]
    \centering
    \begin{tikzpicture}
      \shade[ball color=orange] (0,0) circle (.2) node[above=2mm] {$q$};
      \draw[thin,fill=orange!20] (2.8,-.18) rectangle (8.2,.18);
